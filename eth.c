  ...
  mac_addr_t src, uint16_t type, unsigned char buffer[], long int timeout )
{
  ...
  do {
    ...
    eth_frame_ptr = (struct eth_frame *) eth_buffer;
      //is_my_mac = memcmp(UNICAST?) || memcmp(MULTICAST?)(comprobar la clase sea tipo D)
    is_my_mac = ((memcmp(eth_frame_ptr->dest_addr,iface->mac_address, MAC_ADDR_SIZE) == 0) || (eth_frame_ptr->dest_addr[0] & 0x01) == 0x01);
